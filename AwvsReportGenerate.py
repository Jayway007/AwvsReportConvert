...
def get_detail(soup):
    ...
    len(list(tables))
    for table in tables:
        # vl_path = table.select('tr')[0].string.strip()  #dev
        scan_url = url
        vl_path = table.select('tr > td > b')[0].string.strip()  #aff
        vl_name = table.select('tr:nth-of-type(2) > td > b')[1].string.strip()  #aff ?????
        # vl_name=table.select('tr > td > b')[2].string.strip() #aff ??????
